...
export const runCommandTool: Tool<RunCommandResult> = {
  definition: {
    ...
    timeoutMs: 5_000,
    version: "1.0.0",
    tags: ["system", "shell", "command", "terminal", "runtime"],
    inputSchema: {
      type: "object",
      properties: {
        command: {
  ...
  async execute(
    ...
    try {
      ...
      const command = nodeInput.command;

      if (!command) {
        return {
          success: false,
          error: "Missing required input: command",
          metadata: {
            tool: runCommandName,
          },
        };
      ...
      return {
        success: false,
        error: error?.message || "Unknown shell execution error",
        metadata: {
          tool: runCommandName,
          executionId: context.runtime.executionId,
